...
const useWeeklyCalendarViewModel = () => {
  ...
  const calculatedWeeklyCalendar: null | Calendar = useMemo(() => {
  ...
  const init = async () => {
    await setWeeklyCalendar()
  }

  return {
    calculatedWeeklyCalendar,
    currentDate,
    init,
  }
}

export default useWeeklyCalendarViewModel
